...
| 3307 | TCP | MySQL（宿主机端口，本项目用） | ❌ 不开放 |
| 6380 | TCP | Redis（宿主机端口，本项目用） | ❌ 不开放 |

> 💡 **关于端口号的说明**：
> - 本项目 `.env` 里 MySQL 宿主机端口设的是 **3307**、Redis 是 **6380**（避免和本地默认服务冲突），所以表格里写 3307/6380。
> - 如果你的 `.env` 用的是默认 3306/6379，那就把表格里换成 3306/6379。
> - **但本质上无所谓**：因为这一行就是"❌ 不开放"，写什么端口都不会暴露出去。
> - 真正重要的是：**绝对不要在安全组里放行 3306/3307/6379/6380/3000**，这些只走容器内网即可，否则几小时内必被挖矿/勒索。
...
> 完成以上步骤后，你的 AiTrainer 就是一个**生产级**的线上服务了 🎉
>
> 如有问题随时回到本手册排查，或者让 AI 助手协助。
